 ...
 */
function convertDocsJsonToMarkdown(docData) {
    let markdown = '';
 ...
 */
function convertTextRunToMarkdown(textRun) {
...
server.addTool({
    ...
    execute: async (args, { log }) => {
        ...
        try {
            ...
            const request = { insertText: { location, text: textToInsert } };
    ...
    execute: async (args, { log }) => {
        ...
        try {
            if (args.tabId) {
                ...
                const request = { insertText: { location, text: args.textToInsert } };
                await GDocsHelpers.executeBatchUpdate(docs, args.documentId, [request]);
            }
            else {
...
server.addTool({
    ...
    execute: async (args, { log }) => {
        ...
        try {
            ...
            const range = { startIndex: args.startIndex, endIndex: args.endIndex };
            if (args.tabId) {
    ...
    execute: async (args, { log }) => {
        ...
        try {
            ...
            if ('textToFind' in args.target) {
                ...
                const textRange = await GDocsHelpers.findTextRange(docs, args.documentId, args.target.textToFind, args.target.matchInstance || 1);
                if (!textRange) {
...
server.addTool({
    ...
    execute: async (args, { log }) => {
        ...
        try {
            ...
            await GDocsHelpers.insertInlineImage(docs, args.documentId, imageUrl, args.index, args.width, args.height);
            let sizeInfo = '';
            if (args.width && args.height) {
                sizeInfo = ` with size ${args.width}x${args.height}pt`;
            }
    ...
    execute: async (args, { log }) => {
        ...
        try {
            ...
            if (error instanceof UserError)
                throw error;
            if (error instanceof NotImplementedError)
                throw error; // Expected if helper not implemented
...
server.addTool({
    ...
    parameters: z.object({
        ...
        .refine(data => Object.keys(data).some(key => !['documentId', 'textToFind', 'matchInstance'].includes(key) && data[key] !== undefined), {
    ...
    execute: async (args, { log }) => {
        ...
        try {
            ...
            if (args.linkUrl !== undefined)
                styleParams.linkUrl = args.linkUrl;
            // Find the text range
...
server.addTool({
    ...
    execute: async (_args, { log }) => {
        log.info('Returning highlighted text spans for extract_highlights');
      ...
      content: [
        {
            ...
            text: JSON.stringify(
                {comments:[
                    ...
                    comment: 'a simple burial'
                    },
                    {
                    start_index: 123,
                    associated_text: 'They fell with a hiss',
                    comment: 'why a hiss?'
                    },
                    {
                    start_index: 210,
                    associated_text: 'and his back like',
                    comment: 'beaklike'
                    },
                    {
                    start_index: 168,
                    associated_text: 'the white feathers between his toes',
                    comment: 'how did the cat die? killed by a bird? did it kill a bird?'
                    },
                    {
                    start_index: 262,
                    associated_text: 'There are sorrows keener than these.',
                    comment: 'sharp, biting'
                    },
                    {
                    start_index: 262,
                    associated_text: 'There are sorrows keener than these.',
                    comment: 'losing a pet not as bad as losing another person'
                    },
                    {
                    start_index: 382,
                    associated_text: 'burbles from a dripping bush',
                    comment: 'sing happily'
                    },
                    {
                    start_index: 382,
                    associated_text: 'burbles from a dripping bush',
                    comment: 'the singing robin didn’t make the narrator feel any better – kind of annoyed her'
                    }
                ]
            }
            ), 
